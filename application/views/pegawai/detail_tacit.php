                ...
                <div class="col-lg-10 col-lg-offset-1">
                    <div class="ibox">
                        ...
                        <div class="ibox-content">
                            <?php foreach ($tacit as $key => $value) { ?>
                            ...
                            <div id="share-buttons">
                                <!-- Google+ -->
                                <a href="https://plus.google.com/share?url=<?php echo site_url('daftar_tacit/detail/'.$value['id_tacit']); ?>" target="_blank">
                                    <img src="https://simplesharebuttons.com/images/somacro/google.png" alt="Google" />
                                </a>
                                 
                                <!-- Twitter -->
                                <a href="https://twitter.com/share?url=<?php echo site_url('daftar_tacit/detail/'.$value['id_tacit']); ?>" target="_blank">
                                    <img src="https://simplesharebuttons.com/images/somacro/twitter.png" alt="Twitter" />
                                </a>

                                <!-- Facebook -->
                                <a href="http://www.facebook.com/sharer.php?u=<?php echo site_url('daftar_tacit/detail/'.$value['id_tacit']); ?>" target="_blank">
        ...
        <script>
                    ...
                    if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                        ...
                        for(var i = 0; i < objek.length; i++) {
                            if(objek[i].id_pengguna == <?php  echo $this->session->userdata('id_pengguna'); ?>) {
                                var hapus = '<a class="text-danger pull-right" onclick="if (confirm(\'Komentar akan dihapus, apakah Anda yakin?\')) hapuskomentar('+ objek[i].id_komentar_tacit +');return false;"><i class="fa fa-trash"></i> Hapus</a>';
                            }else {
                                var hapus = '';
                            }
                            ...
                            '<div class="social-feed-box" id="'+ objek[i].id_komentar_tacit +'">'+
                                ...
                                '</div>'+
                            '</div>';
                            last = objek[i].id_komentar_tacit;
                            //window.scrollTo(0,document.body.scrollHeight);
                        }
                    }
                }

                xmlhttp.open("GET","<?php echo site_url('daftar_tacit/ambil_komentar_tacit/'.$this->uri->segment(3).'/')?>" + last, true);
                xmlhttp.send();
            }
            ...
            function sendkomentar() {
                ...
                xmlhttp_kirim.open("GET","<?php echo site_url('daftar_tacit/tambah_komentar_tacit/'.$this->uri->segment(3).'/')?>"+ document.getElementById('respon_komentar').value, true);
                xmlhttp_kirim.send();
            }

            function hapuskomentar(id_komentar_tacit) {
                var xmlhttp_kirim;
                if(window.XMLHttpRequest) {
                    // code for IE7+, Firefox, Chrome, Opera, Safari
                    xmlhttp_kirim = new XMLHttpRequest();
                ...
                xmlhttp_kirim.onreadystatechange = function() {
                    if (xmlhttp_kirim.readyState == 4 && xmlhttp_kirim.status == 200) {
                        document.getElementById(id_komentar_tacit).remove();
                    }
                }

                xmlhttp_kirim.open("GET","<?php echo site_url('daftar_tacit/hapus_komentar_tacit/')?>" + id_komentar_tacit, true);
                xmlhttp_kirim.send();
            }
        </script>  